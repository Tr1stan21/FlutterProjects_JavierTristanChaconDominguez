import 'package:flutter/material.dart';

//
// ────────────────────────────────────────────────────────────────
//   PALETA OSCURA
// ────────────────────────────────────────────────────────────────
//
const Color kPrimaryPurple = Color(0xFF5A2676);
const Color kDarkBackground = Color(0xFF120718);
const Color kSurfacePurple = Color(0xFF1E0D2C);
const Color kGoldAccent = Color(0xFFD4AF37);
const Color kTextPrimary = Color(0xFFFDF7FF);
const Color kTextSecondary = Color(0xFFC7B4D9);

//
// ────────────────────────────────────────────────────────────────
//   PALETA CLARA (Optimizada)
// ────────────────────────────────────────────────────────────────
//
const Color kLightBackground = Color(0xFFFCFAFF);
const Color kLightSurface = Color(0xFFF1E7FF);
const Color kLightTextPrimary = Color(0xFF2B0E3A);
const Color kLightTextSecondary = Color(0xFF6D5A80);
const Color kLightIconColor = kPrimaryPurple;
